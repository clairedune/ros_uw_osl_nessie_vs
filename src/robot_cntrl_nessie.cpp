...
ros::Publisher pub;
int h_min, h_max, lowThreshold, dilatek, erodek;
int counter = 0;
int const max_lowThreshold = 100;

void on_trackbar_erodek( int, void* )
{
}
void on_trackbar_dilatek( int, void* )
{
}
...
void cannyDetector(cv::Mat src, cv::Rect& bRect)
{
	...
	cv::erode(imgMap, imgMap, getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(erodek, erodek)));
	cv::dilate(imgMap, imgMap, getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(dilatek, dilatek)));
...
void motionControl(cv::Mat img, float xi_d, float yi_d, cv::Rect bRect)
{
	// declare commands
	int ball_size = 20000; // in pixels
	float dx, dy, dz;
	float kp, kpz; kp = 0.001; kpz = 0.00001;
	geometry_msgs::TwistStamped ts;
	// if ball is enough large send command
	if(bRect.width*bRect.height > 0.1*ball_size && abs(bRect.width - bRect.height) < 20)
	{
	...
	else
	{
		...
		ts.twist.linear.z = 0;
		pub.publish(ts);
		std::cerr<<"NO VISUAL INFORMATION... COMMAND NULL"; //command
	}
	std::cerr << std::endl;
	counter++;
}
...
void imageCallBack(const sensor_msgs::ImageConstPtr& msg) {
	...
	cannyDetector(img, bRect);
	// robot motion control
	motionControl(img, 0.5*(img.cols),0.5*(img.rows),bRect);
	//visualization
	circle(img, cv::Point(0.5*(img.cols),0.5*(img.rows)), 10, cv::Scalar(0,255,0), 1, 8);
	cv::imshow("view", img);
	cv::waitKey(10);
	cv::imwrite("/home/matheus/nessie.jpg",img);
...
int main(int argc, char **argv) {
	...
	h_min = 5;
	h_max = 30;
	erodek = 15; dilatek = 25;
	lowThreshold = 0;
	cv::createTrackbar( "erodek", "imgCanny", &erodek, 100, on_trackbar_erodek );
	cv::createTrackbar( "dilatek", "imgCanny", &dilatek, 100, on_trackbar_dilatek );
	//cv::createTrackbar( "min Hue", "imgCanny", &h_min, 180, on_trackbar_min );
	//cv::createTrackbar( "max Hue", "imgCanny", &h_max, 180, on_trackbar_max );
